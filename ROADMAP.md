...
- `_guides/nola-group-marijuana-laws-guide.md` — targets "is weed legal in new orleans" / "can you smoke marijuana in new orleans" — Louisiana's decriminalized-not-legal status for small amounts (civil fine rather than arrest, no invented dollar figures), how that differs from actual legality, where public consumption still isn't allowed (same public-space logic as the open container rule), and how it plays out differently for a villa-style rental vs. a hotel room; checked `_guides/` — no dedicated page exists, only one unrelated incidental mention
