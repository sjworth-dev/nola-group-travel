...
- **PRIORITY REFRESH FLAG (not a new-content item):** `_guides/nola-group-hurricane-season-guide.md` — this guide currently frames "August-October" as "shoulder season" in its title, description, section headers, checklist, and final CTA. That conflicts directly with this file's own Seasonality Ground Truth: October is peak (not shoulder), and the only genuine shoulder/value window is late August through mid-September. It also conflates the hurricane risk window with a value-season claim, which the Ground Truth section explicitly warns against. This needs a full pass rewriting the seasonal framing throughout (not a one-line fix) — next run should pick this for the Refresh step ahead of the mechanically-oldest-dated guide, given the scope of the live inaccuracy.
