...
- **PRIORITY REFRESH FLAG (not a new-content item):** `_guides/shoulder-season-guide.md` (title/intro/table) and `_guides/nola-group-october-guide.md` (description, card_description, intro, checklist all claim October has "lower rates"/"soft prices") both frame October as a bargain/shoulder month. That conflicts with this file's own Seasonality Ground Truth: October is peak (Voodoo Fest, Halloween week, best weather), not shoulder — the only genuine shoulder/value window is late August through mid-September. Found while fixing the same error class in `nola-group-hurricane-season-guide.md` (now corrected). Next run should Refresh one of these two ahead of the mechanically-oldest-dated guide, given the scope of the live inaccuracy and that it's now inconsistent with the corrected hurricane-season guide, which links to both.
