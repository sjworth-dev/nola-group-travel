...
- `_guides/nola-group-red-dress-run-guide.md` — targets "red dress run new orleans group" / "red dress run nola" — the Red Dress Run is a real, huge annual Hash House Harriers charity pub-crawl/run held in high summer (August) where the entire crowd, all genders, wears red dresses for a costumed romp through the French Quarter/CBD; a distinctive soft-season group activity in the same participatory-street-event vein as the Running of the Bulls, and a natural companion to it for an August crew; write generally on format/what-to-expect/dress-code and defer exact date/route/registration to official sources; checked `_guides/` — only an incidental mention in shoulder-season-guide.md, no dedicated page; category "Festivals & Events"
