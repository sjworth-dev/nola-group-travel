...
- `_guides/nola-group-new-orleans-vs-austin-guide.md` — targets "new orleans vs austin bachelorette" / "new orleans vs austin group trip" — live music (jazz/brass vs. Austin's scene), food culture, lake/river day activities vs. NOLA's villa-pool culture, large-group rental landscape, heat and humidity comparison, verdicts by trip type; same comparison format as the Nashville and Charleston guides, no existing Austin comparison in `_guides/`
- `_guides/nola-group-college-friends-reunion-guide.md` — targets "college friends reunion trip new orleans" / "new orleans reunion trip old friends" — the specific dynamics of a crew that hasn't traveled together in years (catching up time vs. activity time, mixed budgets/life stages, the "who's changed" factor); NOTE: friends-trip.md has a short existing "Friend Reunion" section — before writing, confirm this guide clears 30%+ new material as a standalone deep dive rather than restating that section; if it can't, expand friends-trip.md instead
- `_guides/nola-group-open-container-law-guide.md` — targets "can you drink on the street in new orleans" / "new orleans open container law" — the actual to-go cup rule (plastic only, no glass/cans), where it applies vs. doesn't (French Quarter and the parade routes vs. residential streets), how this changes group bar-crawl and pool-day logistics; mentioned only in passing across bar-crawl-guide.md, nightlife-guide.md, and others — no guide targets this exact question directly, and it's the literal example query named in this file's own Topic Selection Bar section
...
- **PRIORITY REFRESH FLAG (not a new-content item):** `_guides/shoulder-season-guide.md` (title, intro, "October: The Best Month Nobody Talks About" section, and both rate tables) still frames October as a bargain/shoulder month. That conflicts with this file's own Seasonality Ground Truth: October is peak (Voodoo Fest, Halloween week, best weather), not shoulder — the only genuine shoulder/value window is late August through mid-September. `nola-group-october-guide.md` had the same error and was corrected 2026-07-16 (pricing section, description/card_description, intro, and Where to Stay section rewritten to frame October as peak, not a discount month). Next run should Refresh `shoulder-season-guide.md` ahead of the mechanically-oldest-dated guide — it's now the last file still making this claim, and it's inconsistent with both the corrected hurricane-season guide and the corrected October guide, which link to it.
