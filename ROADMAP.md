...
- `_guides/nola-group-red-dress-run-guide.md` — targets "red dress run new orleans group" / "red dress run nola" — the Red Dress Run is a real annual New Orleans charity fun-run (Hash House Harriers) held in August where everyone, regardless of gender, wears a red dress and the "run" is really a walking bar crawl/street party through the French Quarter and Marigny; a distinctive costume-driven group event sitting squarely in the August soft/value season per the Seasonality section (pairs with the white-linen-night August-value angle); write generally on format/dress code/charity framing and defer exact date/route to the official site; checked `_guides/` — no page exists, category "Festivals & Events"

- `_guides/nola-group-reveillon-dinner-guide.md` — targets "reveillon dinner new orleans group" / "reveillon new orleans christmas" — Reveillon is New Orleans' Creole holiday-dinner tradition where restaurants run special multi-course prix-fixe menus through December; a natural anchor for a big group's December trip and a strong complement to the existing new-years-guide/new-years-villa-guide (link out, don't duplicate); write generally on the tradition, the multi-course format, and how to book a big group around it, deferring specific restaurants/menus/prices to official sources per the no-invented-specifics rule; checked `_guides/` — no dedicated page, category "Food & Drink"

- `_guides/nola-group-bayou-boogaloo-guide.md` — targets "bayou boogaloo new orleans group" / "bayou boogaloo mid-city" — Bayou Boogaloo is a real free spring music-and-arts festival on the banks of Bayou St. John in Mid-City, with the distinctive angle that people watch from the water on floats/kayaks/tubes; a spring high-season (May) group anchor in a neighborhood the site undercovers; write generally on format/layout/the-on-water element and defer current-year lineup/dates to official sources; checked `_guides/` — no page exists, category "Festivals & Events"
